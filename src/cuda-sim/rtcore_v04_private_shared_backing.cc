...
static bool owner_is_valid(
    ...
    uint32_t owner_hw_sid, uint8_t resident_warp_slot, uint8_t lane) {
...
static bool same_operation(const shared_write_v0 &lhs,
  ...
  return lhs.valid == rhs.valid &&
         ...
         lhs.reserved_zero == rhs.reserved_zero &&
...
static bool has_in_flight_owner(
  ...
  if (state.offer_valid &&
      operation_belongs_to_owner(state.offered, owner)) {
    return true;
  }
...
static bool all_init_chunks_acknowledged(
  ...
  const uint32_t expected =
      (uint32_t{1} << lane.init_plan.access_count) - 1;
  return lane.acknowledged_chunk_mask == expected;
}

static void materialize_operation(
    const lane_slot_state_v0 &lane, uint8_t chunk_id,
    uint64_t enqueue_cycle, shared_write_v0 *operation) {
  ...
  const uint32_t chunk_slot_base =
      access.slot_byte_offset - first_byte;
  for (uint32_t byte = 0;
       byte < private_frontier::kSharedAccessChunkBytes; ++byte) {
    if ((access.byte_mask & (uint32_t{1} << byte)) == 0) continue;
    operation->payload[byte] =
        lane.staging_slot.bytes[chunk_slot_base + byte];
  }
}

static bool validate_operation_against_lane(
    const shared_write_v0 &operation,
    const lane_slot_state_v0 &lane) {
  if (!operation.valid ||
      operation.memory_op_seq != kInitMemoryOpSequence ||
  ...
  materialize_operation(lane, operation.chunk_id,
                        operation.enqueue_cycle, &expected);
...
static status_kind validate_release_lanes(
    const backing_state_v0 &state,
    const resident_warp_state_v0 &warp, uint32_t release_mask) {
...
status_kind prepare_new_warp(
  ...
  for (uint32_t lane = 0; lane < kLaneCapacity; ++lane) {
    ...
    if (resident_slot == kResidentWarpCapacity) {
      resident_slot =
          static_cast<uint8_t>(owners[lane].resident_warp_id);
  ...
  for (uint32_t slot = 0; slot < kResidentWarpCapacity; ++slot) {
    if (warp_matches(state.resident_warps[slot], state.owner_hw_sid,
                     warp_uid, warp_id)) {
      return kStatusDuplicateWarp;
    }
  }
  ...
  for (uint32_t lane = 0; lane < kLaneCapacity; ++lane) {
    ...
    for (uint32_t slot = 0; slot < kResidentWarpCapacity; ++slot) {
      const resident_warp_state_v0 &live_warp =
          state.resident_warps[slot];
      if (!live_warp.live) continue;
      for (uint32_t live_lane = 0; live_lane < kLaneCapacity;
           ++live_lane) {
  ...
  if (state->owner_hw_sid != plan.owner_hw_sid ||
      ...
      plan.charged_bytes !=
          count_lanes(plan.active_mask) *
              kResidentChargeBytesPerLane ||
      plan.charged_bytes >
          kResidentSharedBytes - state->charged_bytes) {
    return kStatusOwnerMismatch;
  }
...
uint32_t service_init_enqueue(backing_state_v0 *state,
  ...
  for (uint32_t warp_slot = 0;
    ...
    for (uint32_t lane = 0;
         lane < kLaneCapacity && enqueued < enqueue_budget; ++lane) {
      ...
      while (slot.live &&
        ...
        materialize_operation(slot, slot.next_chunk_to_enqueue,
        ...
        slot.enqueued_chunk_mask |=
            uint32_t{1} << slot.next_chunk_to_enqueue;
...
uint32_t service_write_acks(backing_state_v0 *state,
  ...
  while (!state->outstanding.empty() &&
    ...
    if ((lane.acknowledged_chunk_mask &
         (uint32_t{1} << operation.chunk_id)) != 0) {
    ...
    lane.acknowledged_chunk_mask |=
        uint32_t{1} << operation.chunk_id;
    ...
    if (!warp.scheduler_ready) {
      bool complete = true;
      for (uint32_t warp_lane = 0; warp_lane < kLaneCapacity;
           ++warp_lane) {
        if ((warp.active_mask & lane_bit(warp_lane)) == 0) continue;
        complete =
            complete &&
            all_init_chunks_acknowledged(warp.lanes[warp_lane]);
      }
      if (complete) {
...
bool pop_ready_commit(backing_state_v0 *state,
  ...
  if (slot >= kResidentWarpCapacity ||
      !state->resident_warps[slot].live ||
...
status_kind prepare_mask_shrink(
  ...
  if (!warp_matches(warp, state.owner_hw_sid, previous_warp_uid,
                    warp_id) ||
  ...
  const status_kind revalidation_status =
      prepare_mask_shrink(
          *state, plan.resident_warp_slot, plan.previous_warp_uid,
          plan.next_warp_uid, plan.warp_id, plan.next_active_mask,
          &revalidated);
  if (revalidation_status != kStatusOk) {
    return revalidation_status;
  }
...
status_kind commit_release_warp(backing_state_v0 *state,
  ...
  const status_kind revalidation_status =
      prepare_release_warp(*state, plan.resident_warp_slot,
  ...
  resident_warp_state_v0 &warp =
      state->resident_warps[plan.resident_warp_slot];
  if (!warp_matches(warp, plan.owner_hw_sid, plan.warp_uid,
                    plan.warp_id) ||
...
const lane_slot_state_v0 *find_live_lane(
  ...
  return lane.live && private_frontier::owners_equal(lane.owner, owner)
             ? &lane
             : NULL;
}
